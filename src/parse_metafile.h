...
int read_metafile(char *metafile_name);             //读取种文件
int find_keyword(char *keyword, long *position);    //在种子文件中查询某个关键词
int read_announce_list();                           //获取各个tracker服务器的地址
int add_an_announce(char *url);                     //向tracker列表添加一个URL

int get_piece_length();     //获取各个piece的长雅, 一般为256kb
...
int parse_metafile(char *metafile);         //调用本文件中定义的函数,完成解释种子文件

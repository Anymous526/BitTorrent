...
int print_bitfield(Bitmap *bitmap); //打印位置值,用于调试

int restore_bitmap();                       //将位图存储到文件中 ,在下次下载时,先读取该文件获取已经下载的进度
int is_interested(Bitmap *dst, Bitmap *src);    //拥有位图src的peer 是否拥有dst位位的peer感兴趣
int get_download_piece_num();                   //获取当前已经下载的总piece数
